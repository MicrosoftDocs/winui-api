...
The rendering position for an object can be offset on a [Canvas](../microsoft.ui.xaml.controls/canvas.md) using [Canvas.Left](/uwp/api/microsoft.ui.xaml.controls.canvas#xaml-attached-properties) and [Canvas.Top](/uwp/api/microsoft.ui.xaml.controls.canvas#xaml-attached-properties), but this does not count as a transformation; the object retains its own local (0,0) origin when it's positioned in a [Canvas](../microsoft.ui.xaml.controls/canvas.md).
