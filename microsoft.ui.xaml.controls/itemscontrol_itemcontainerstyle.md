...
## -description
Gets or sets the style that is used when rendering the item containers for an [ItemsControl](/uwp/api/microsoft.ui.xaml.controls.itemscontrol). 
...
Derived classes of `ItemsControl` include [Selector](/uwp/api/microsoft.ui.xaml.controls.primitives.selector) and [MenuFlyoutPresenter](/uwp/api/microsoft.ui.xaml.controls.menuflyoutpresenter). Derived classes from Selector include [ComboBox](/uwp/api/microsoft.ui.xaml.controls.combobox), [FlipView](/uwp/api/microsoft.ui.xaml.controls.flipview), [ListBox](/uwp/api/microsoft.ui.xaml.controls.listbox), and [ListViewBase](/uwp/api/microsoft.ui.xaml.controls.listviewbase) (which [ListView](/uwp/api/microsoft.ui.xaml.controls.listview) and [GridView](/uwp/api/microsoft.ui.xaml.controls.gridview) are derived from). Therefore the `ItemContainerStyle` class can be used to edit the [Style](/uwp/api/microsoft.ui.xaml.style) of a `ListViewItem`, `GridViewItem`, and so on. 
...
`ItemContainerStyle` can then be used to change the style of every element generated by an ItemsControl, and provides a lot of flexibility and customization options in that area. Although many styling options are available within the `ItemContainerStyle` class, it is most useful when trying to edit how the _elements containing the data_ are displayed in comparison to one another - changing their spacing, their horizontal alignment, etc. To change the visualization of the _data objects themselves_, such as their background color or font-size, use an [ItemTemplate](/uwp/api/microsoft.ui.xaml.controls.itemscontrol.itemtemplate). To change the overall layout or orientation of your ItemsControl/its items, look into editing its [ItemsPanel](/uwp/api/microsoft.ui.xaml.controls.itemscontrol.itemspanel). 
...
These customizations are performed by creating a `ListViewItem` style, as shown above. The style element targets `ListViewItem` type elements, and will be applied to all elements of that type within the scope that the style is defined in - in this case, just the `ListView` displayed. [Setters](/uwp/api/microsoft.ui.xaml.setter) are then used to give each style property a value - i.e. set the `HorizontalAlignment` to `Right`. These properties and values are what affects the outer `ListViewItem` element.
