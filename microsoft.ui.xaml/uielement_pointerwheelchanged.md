...
When you handle PointerWheelChanged, you are usually interested in the [MouseWheelDelta](/uwp/api/windows.ui.input.pointerpointproperties.mousewheeldelta) property value:

1. Call [GetCurrentPoint](/uwp/api/windows.ui.xaml.input.pointerroutedeventargs.getcurrentpoint(windows.ui.xaml.uielement)) from the [PointerRoutedEventArgs](../microsoft.ui.xaml.input/pointerroutedeventargs.md) available to your handler. This returns a [PointerPoint](/uwp/api/windows.ui.input.pointerpoint).
1. Get the [Properties](/uwp/api/windows.ui.input.pointerpoint.properties) of the [PointerPoint](/uwp/api/windows.ui.input.pointerpoint). It contains a [PointerPointProperties](/uwp/api/windows.ui.input.pointerpointproperties) object.
1. Get the [MouseWheelDelta](/uwp/api/windows.ui.input.pointerpointproperties.mousewheeldelta) property value from the [PointerPointProperties](/uwp/api/windows.ui.input.pointerpointproperties) object. A positive value indicates that the wheel was rotated forward (away from the user) or tilted to the right; a negative value indicates that the wheel was rotated backward (toward the user) or tilted to the left.
