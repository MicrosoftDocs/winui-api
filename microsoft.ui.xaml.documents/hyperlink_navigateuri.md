...
If your intent is that the [Hyperlink](hyperlink.md) should load a specified URI within a [WebView](/uwp/api/windows.ui.xaml.controls.webview) control that's also part of your app, then don't specify a value for NavigateUri. Handle the [Click](hyperlink_click.md) event instead, and call [WebView.Navigate](/uwp/api/windows.ui.xaml.controls.webview.navigate(windows.foundation.uri)), specifying the URI to load.
