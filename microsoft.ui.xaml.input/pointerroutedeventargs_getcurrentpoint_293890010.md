...
## -description
Retrieves a [PointerPoint](/uwp/api/windows.ui.input.pointerpoint) object that provides basic info on the pointer associated with the event.
...
## -returns
A [PointerPoint](/uwp/api/windows.ui.input.pointerpoint) value that represents the pointer point associated with this event. If **null** was passed as *relativeTo*, the coordinates are in the frame of reference of the overall window. If a *relativeTo* value other than **null** was passed, the coordinates are relative to the object referenced by *relativeTo*.
